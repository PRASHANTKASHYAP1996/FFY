...
class CallkitSoundPlayerManager(private val context: Context) {
    ...
    private var playingCallId: String = ""
    ...
    fun play(data: Bundle) {
        val callId = data.getString(CallkitConstants.EXTRA_CALLKIT_ID, "").trim()
        if (isPlaying && callId.isNotEmpty() && playingCallId == callId) {
            android.util.Log.d("CallkitRingtone", "ringtone.duplicate_start_ignored")
            return
        }
        this.isPlaying = true
        ...
        this.playSound(data)
        this.playVibrator()
        android.util.Log.d("CallkitRingtone", "ringtone.start")
    ...
    fun stop() {
        if (!isPlaying && ringtone == null && vibrator == null) {
            android.util.Log.d("CallkitRingtone", "ringtone.stop_ignored_not_playing")
            return
        }
        ...
        android.util.Log.d("CallkitRingtone", "ringtone.stop")
    }
